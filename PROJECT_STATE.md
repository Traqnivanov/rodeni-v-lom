...
1. Прочети `CLAUDE.md`.
2. Прочети този файл.
3. Прочети само релевантните канонични части от `PRODUCT_MASTER_VISION_AUDIT.md`.
4. Провери актуалния `main` и, ако задачата е DB/security/auth, реалното състояние на Supabase `mqilvavuzbsscsfhtuub`.
5. Продължи от **NEXT EXACT STEP**. Не започвай нов общ одит без конкретна причина.
